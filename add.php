...
if(isset($_POST['addBtn'])){
   ...
   if(!empty($newItem) && !empty($itemQuantity) && !empty($price)){

    $totlaPrice = $itemQuantity * $price;
    $sql = "INSERT INTO `add_items`(`item_name`, `quantity`, `price`, `totalPrice`) VALUES ('$newItem','$itemQuantity','$price','$totalPrice')";

    $query = mysqli_query($conn,$sql);
    if($query){
        header("location: add.php?message= ".$newItem." ".$itemQuantity." kg was  added successfully!");
    }
    else{
        header("location: add.php?messageErro= new Product failed to be added!");
    }
   }
}
        ...
        <form method="POST">
         <table>
            ...
            <tr>
                ...
                <td><input type="number" placeholder="price for 1kg" name="price" required></td>
            </tr>  
         </table><br>
        </div>
        </form>
